...
def get_df_price(stock, folder="./tencent_data"):
    
    ends = []
 
    # 打开本地文件
    try:
        file = open(folder + "/" + stock.upper() + ".json")
    except BaseException:
        print("File open error for {}!".format(stock))
        return None
    ...
    # 数据（截止前一天）
    if "qfqday" in response_dict["data"][stock.lower()].keys():
        data = response_dict["data"][stock.lower()]["qfqday"]
    else:
        data = response_dict["data"][stock.lower()]["day"]
    ...
    # 添加最新数据
    ends.append(float(response_dict["data"][stock.lower()]["qt"][stock.lower()][3]))
